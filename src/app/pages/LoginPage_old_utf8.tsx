...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen flex batik-pattern">
      {/* Left ΓÇö Branding (heritage hero) */}
      <div className="hidden lg:flex flex-col justify-between w-[460px] bg-sidebar text-sidebar-foreground p-12 relative overflow-hidden">
        ...
        <div className="relative flex items-center gap-3">
          <img src={logoImg} alt="Pawon Salam" className="w-11 h-11 rounded-xl object-cover ring-1 ring-gold/40" />
          <div>
            <p className="font-display text-lg leading-none text-sidebar-foreground">Pawon Salam</p>
        ...
        <div className="relative space-y-10">
          <div>
            ...
            <p className="text-sidebar-foreground/70 text-sm mt-5 leading-relaxed max-w-sm">
              Platform manajemen restoran terintegrasi ΓÇö dari pemesanan tamu melalui QR, dapur, hingga laporan penjualan.
            </p>
          </div>
        ...
        <p className="relative text-[11px] tracking-widest uppercase text-sidebar-foreground/40">┬⌐ 2025 Pawon Salam Resto</p>
      </div>

      {/* Right ΓÇö Login Form */}
      <div className="flex-1 flex items-center justify-center p-6">
        <div className="w-full max-w-md">
          {/* Mobile logo */}
          <div className="flex lg:hidden items-center justify-center gap-3 mb-10">
            <img src={logoImg} alt="Pawon Salam" className="w-12 h-12 rounded-xl object-cover ring-1 ring-gold/40" />
            <div>
              <p className="font-display text-xl leading-none">Pawon Salam</p>
              <p className="eyebrow mt-1">Restaurant System</p>
            </div>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              ...
              <button
                type="button"
                onClick={() => { setPassword(CREDENTIALS[role].password); setError(""); }}
                className="mt-2 text-[11px] text-gold hover:text-primary transition-colors tracking-wide"
              >
                Gunakan password default: <span className="font-mono">{CREDENTIALS[role].password}</span>
              </button>
            </div>
